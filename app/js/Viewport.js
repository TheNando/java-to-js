...
class Viewport {
  constructor(width, height, screenWidth, screenHeight, graphics) {
    ...
    // https://github.com/CIS580/double-buffer/blob/master/game.js
    // game.buffer = document.getElementById("buffer");
    // game.bctx = game.buffer.getContext("2d");  
    // game.canvas = document.getElementById("canvas");
    // game.ctx = game.canvas.getContext("2d");
    // game.bctx.createImageData(game.width, game.height);
    // game.bctx.font = "bold 14px sans-serif";
    // game.bctx.fillStyle = "white";
  ...
  generateBrightness() {
    ...
    for (i = 0; i < 512; i++) {
      ...
      if (i < 4) this.brightness[i] = this.brightness[i] * i / 4;
    }
  }
  ...
  drawBulletTrace(cos, sin, closestHitDist) {
    ...
    for (i = closestHitDist; i >= 0; i--) {
      ...
      if (this.isWithinView(xm, ym)) {
        ...
        this.lightmap[xm + ym * width] +=
          glow * (255 - this.lightmap[xm + ym * width]) / 255;
      }
  ...
  drawBulletDebris(playerDir, hitMonster, hitPoint) {
    ...
    for (i = 0; i < 10; i++) {
      ...
      xd =
        Math.floor(hitPoint.x - Math.cos(playerDir + dir) * pow) +
        this.random.nextInt(4) -
        this.random.nextInt(4);
      yd =
        Math.floor(hitPoint.y - Math.sin(playerDir + dir) * pow) +
        this.random.nextInt(4) -
        this.random.nextInt(4);
    ...
    for (x = -12; x <= 12; x++) {
      for (y = -12; y <= 12; y++) {
        ...
        if (
          ...
          this.lightmap[offsetPoint.x + offsetPoint.y * width] +=
            2000 /
            (x * x + y * y + 10) *
            (255 - this.lightmap[offsetPoint.x + offsetPoint.y * width]) /
            255;
        }
      }
    }
  ...
  handleShot(
    ...
    if (closestHitDistance < width_half) {
      ...
      hitPoint = new Point(
        Math.floot(width_half + cos * closestHitDistance),
        Math.floot(height_half - sin * closestHitDistance)
      );
