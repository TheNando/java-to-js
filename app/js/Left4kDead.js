...
class Left4kDead {
  ...
  processMonster(viewport, tick, monster, playerDir, cos, sin, camera) {
    ...
    if (!monster.isActive()) {
      ...
      if (
        !this.map.isMonsterHead(xPos, yPos) &&
        (monster.isEarly() ||
          this.game.getRushTime() > 0 ||
          (monster.isSpecial() && tick == 1))
      ) {
        monster.place(
          xPos,
          yPos,
  ...
  doOneMovementIteration(
    ...
    if (this.didMove(movement)) {
      ...
      for (
        xx = monster.position.x + movement.x - 3;
        xx <= monster.position.x + movement.x + 3;
        xx++
      ) {
        for (
          yy = monster.position.y + movement.y - 3;
          yy <= monster.position.y + movement.y + 3;
          yy++
        ) {
